    ...
    if data.get('type') == 'trade':
        for trade in data['data']:
            ...
            if ticker in history_data:
                ...
                # 최근 360초(6분) 데이터만 유지 (이전 5분 평균 거래량 비교용)
                while history and (now - history[0][0] > 360):
                    history.popleft()
                
                # 1분(60초) 이내 데이터와 이전 5분(300초) 데이터 분리
